...
- **`attendanceController.js`** — `listAttendance` (computes percentage, safe/warning/shortage state, and a "Safe Classes" calculator per record: how many classes can be safely missed above the threshold, or how many must be attended next to recover), `markAttendance` (the upsert, already validates subject ownership directly), `deleteAttendance`.
