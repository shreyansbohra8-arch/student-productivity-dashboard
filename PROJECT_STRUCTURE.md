...
- **`analyticsController.js`** — Seven aggregation pipelines feeding the analytics dashboard (see below), including a `$facet`-based split of attendance records into "all" and "below threshold" computed entirely in MongoDB, and a separate `countDocuments()` for the true upcoming-exam total (independent of the 5-item preview list).
